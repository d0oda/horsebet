...
def main():
    with get_session() as session:
        rows = session.execute(
            text("SELECT id FROM races WHERE date >= '2026-05-01' AND date <= '2026-05-23' ORDER BY date, race_number")
        ).fetchall()
    race_ids = [r[0] for r in rows]
    print(f"Running predictions on {len(race_ids)} races for May 2026...")
    
    # Predict all horses for May 2026
    df = predict_with_filters(
        race_ids=race_ids,
        model_version="20260523_230830",
        ev_threshold=0.30,  
        max_odds=100.0,      
        min_odds=1.5,         
        bankroll=100000,
    ...
    print("="*50)
    print(f"Total Races Evaluated : {len(race_ids)}")
