"""
UmaEdge — Historical EV Backtesting Engine (Dutching).
...
with support for betting on multiple horses (Dutching).

Usage:
    # Run backtest with latest model and dutching
    python -m models.backtest_dutching --allow-dutching

    # Run with specific model version and EV threshold
    python -m models.backtest_dutching --version 20250101_120000 --ev-threshold 0.10

    # Output detailed results to CSV
    python -m models.backtest_dutching --output results.csv
"""
...
log = logging.getLogger("backtest_dutching")
...
class BacktestConfig:
    ...
    flat_stake: int = 200           # Yen per flat bet
    ...
    allow_dutching: bool = True    # Allow betting on multiple horses per race
...
class Backtester:
    ...
    def run(
        ...
        for race_id, race_df in races:
            ...
            # --- Size bets according to config ---
            bets_to_place = []
            if self.config.allow_dutching:
                for c in candidates:
                    c["stake"] = self.config.flat_stake
                    c["kelly"] = 0
                    bets_to_place.append(c)
            else:
                # Legacy 1-bet limit
                best_c = max(candidates, key=lambda x: x["ev"])
                best_c["stake"] = self.config.flat_stake
                best_c["kelly"] = 0
                bets_to_place.append(best_c)

            # --- Place the bets for this race ---
            if not bets_to_place:
                continue

            race_profit = 0
            for c in bets_to_place:
                stake = c["stake"]
                if stake > balance:
                    continue
                ...
                payout = int(stake * c["odds"]) if won else 0
                profit = payout - stake
                race_profit += profit
            ...
            # Update balance after all bets in the race are settled
            balance += race_profit
            ...
            # Daily P&L
            # Assuming all bets in the race have the same date
            date_str = str(bets_to_place[0]["row"].get("date", "unknown"))
...
def run_full_backtest(
    ...
    if os.path.exists("data/features.parquet"):
        ...
        log.info("features.parquet not found, building from scratch...")
        fb = FeatureBuilder()
    ...
    # To prevent in-sample data leakage (the "pure bullshit" 67% ROI), 
    # we strictly quarantine the backtest to the holdout test set (2026 onwards).
    if "date" in features_df.columns:
        before = len(features_df)
        features_df = features_df[features_df["date"] >= "2026-01-01"].copy()
        log.info(f"Filtered for OOS test set (>=2026): {before} -> {len(features_df)} entries")

    log.info(f"Loading model ({model_version}) and generating predictions...")
    ...
    ensemble_probs = 0.55 * lgb_preds + 0.45 * xgb_preds
    ...
    features_df["_unnorm_prob"] = ensemble_probs
    
    # Normalize per race so probabilities sum to 1.0!
    # Without this, mutually exclusive probabilities can sum to > 1.0, breaking the EV math.
    race_sums = features_df.groupby("race_id")["_unnorm_prob"].transform("sum")
    features_df["win_prob"] = features_df["_unnorm_prob"] / race_sums.replace(0, 1)
...
def main():
    ...
    parser.add_argument("--ev-threshold", type=float, default=0.30, help="Min EV to trigger bet (default: 0.30)")
    ...
    parser.add_argument("--allow-dutching", action="store_true", default=True,
                        help="Allow betting on multiple horses per race (enabled by default in this script)")
    ...
    run_full_backtest(
        ...
        allow_dutching=args.allow_dutching,
    )
