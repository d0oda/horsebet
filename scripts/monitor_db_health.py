...
def check_missing_results():
    """Check for missing finish positions in past races (older than 1 day)."""
    with get_session() as session:
        query = text("""
            SELECT r.date, r.netkeiba_id, COUNT(e.id) as missing_entries
            FROM races r
            JOIN entries e ON r.id = e.race_id
            LEFT JOIN results res ON e.id = res.entry_id
            WHERE r.date < CURRENT_DATE - INTERVAL '1 day'
              AND res.finish_pos IS NULL
    ...
    if rows:
        log.error(f"Found {len(rows)} past races missing finish positions!")
        for r in rows[:5]:
            log.error(f"  - Race {r.netkeiba_id} on {r.date}: {r.missing_entries} entries missing finish_pos")
        return len(rows)
    return 0
